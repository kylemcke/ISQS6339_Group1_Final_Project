# Debug utility - enumerate on every revenue candidate and show best pick
def debug_revenue_candidates(facts: dict, fy: int = 2023) -> dict:
    """
    Return a structured view of all revenue-like candidates considered.
    Shows which pass the FY filter, and the best pick if any.
    """
    report = {"candidates": [], "best": None}
    # Try each revenue tag directly
    for ns, local in TAGS_REVENUE:
        ...
        report["candidates"].extend(tagged)
    # What would pick_revenue select?
    best = pick_revenue(facts, fy=fy)
    report["best"] = best
    return report
...
# Expanded to better support utilities, REITs, and finance variants
TAGS_REVENUE: List[Tuple[str, str]] = [
    ...
    (NS_USGAAP, "RevenueFromContractWithCustomerIncludingAssessedTax"),
    # Utilities
    (NS_USGAAP, "OperatingRevenue"),
    (NS_USGAAP, "RegulatedAndUnregulatedOperatingRevenue"),
    # Real estate / REITs
    (NS_USGAAP, "RentalRevenue"),
    (NS_USGAAP, "RevenueRealEstate"),
    (NS_USGAAP, "OperatingLeaseLeaseIncome"),
    # Finance-oriented combined totals
    (NS_USGAAP, "InterestAndNoninterestIncome"),
    (NS_USGAAP, "TotalRevenue"),
    (NS_USGAAP, "InterestIncomeExpenseAfterProvisionForLoanLoss"),
    (NS_USGAAP, "RevenuesNetOfInterestExpense"),
    (NS_USGAAP, "InterestIncomeOperating"),
    (NS_USGAAP, "InterestAndDividendIncomeOperating"),
    # MISC (Less Common)
    (NS_USGAAP, "TotalRevenues"),
]
...
# Bank/financial fallbacks for revenue
TAGS_BANK_NII: List[Tuple[str, str]] = [(NS_USGAAP, "NetInterestIncome")]
TAGS_BANK_NONINT: List[Tuple[str, str]] = [(NS_USGAAP, "NoninterestIncome")]
...
# Fallback - Financial institution revenue 
def compute_financials_total_revenue(facts: Dict, fy: int = 2023) -> Optional[Dict]:
    """
    For banks/financial institutions that don't report a single 'Revenues' fact,
    calculate total revenue as NetInterestIncome + NoninterestIncome.
    """
    nii  = pick_fact_value(facts, TAGS_BANK_NII, fy=fy)
    noni = pick_fact_value(facts, TAGS_BANK_NONINT, fy=fy)
    if not (nii and noni):
        return None

    try:
        total_val = float(nii["value"]) + float(noni["value"])
    except Exception:
        return None

    return {
        "value": total_val,
        "value_musd": round_millions(total_val, 2),
        "tag": "us-gaap:NetInterestIncome+NoninterestIncome",
        "uom": "USD",
        "accn": nii.get("accn") or noni.get("accn"),
        "filed": nii.get("filed") or noni.get("filed"),
        "form": nii.get("form") or noni.get("form"),
        "fy": fy,
        "fp": "FY",
        "frame": None,
        "start": None,
        "end": None,
        "computed": True,
        "notes": "Computed as NetInterestIncome + NoninterestIncome",
    }

def pick_revenue(facts: Dict, fy: int = 2023) -> Optional[Dict]:
    """
    Try the broader TAGS_REVENUE first. If nothing, use bank/financial fallback.
    """
    rev = pick_fact_value(facts, TAGS_REVENUE, fy=fy)
    if rev:
        return rev
    return compute_financials_total_revenue(facts, fy=fy)
