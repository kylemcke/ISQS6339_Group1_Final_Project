...
from xbrl_extract import (
    ...
    TAGS_NET_INCOME, TAGS_EQUITY, TAGS_CFO,
    pick_revenue,
    debug_revenue_candidates,
)
...
def parse_args():
    ...
    ap.add_argument("--input", type=str, default=DEFAULT_INPUT,
                    help="Path or URL to S&P500 CSV (expects columns: CIK, Symbol, Security).")
    ...
    ap.add_argument("--validate-cik", action="store_true",
                help="If CompanyFacts is empty for the row CIK, retry with ticker→CIK mapping.")


    # User selected output filenames (inside --output-dir)
    ap.add_argument("--output-metrics", type=str, default="sp500_fy2023_metrics.csv",
                    help="Filename for the metrics CSV (inside --output-dir).")
    ap.add_argument("--output-excluded", type=str, default="excluded_non_usd.csv",
                    help="Filename for the excluded (non-USD) CSV (inside --output-dir).")
    ap.add_argument("--output-xlsx", type=str, default="sp500_fy2023_metrics.xlsx",
                    help="Filename for the Excel workbook (inside --output-dir).")
    ap.add_argument("--debug-symbol", type=str, default=None,
                    help="Print revenue tag diagnostics for this ticker symbol and exit.")
...
def load_input_df(path_or_url: str) -> pd.DataFrame:
    df = pd.read_csv(path_or_url, dtype={"CIK":"string","Symbol":"string","Security":"string"})
...
def main():
    ...
    logging.info(f"Loading input CSV from: {args.input}")
    try:
        df_in = load_input_df(args.input)
    except Exception as e:
        logging.exception(f"Failed to load input CSV from {args.input}: {e}")
        sys.exit(1)
    ...
    # Debug revenue tags for a single symbol and exit
    if args.debug_symbol:
        sym = args.debug_symbol.strip().upper()
        logging.info(f"Debugging revenue for symbol: {sym}")
        
        # Resolve row for that symbol
        row = df_in[df_in["Symbol_norm"] == sym].head(1)
        if row.empty:
            logging.error(f"No row found for symbol {sym} in input.")
            sys.exit(2)
        row = row.iloc[0]
        provided_cik = str(row.get("CIK_padded") or "")
        mapped_cik = ticker_map.get(sym)
        for chosen_cik, label in [(provided_cik, "provided CIK"), (mapped_cik, "mapped CIK")]:
            if not chosen_cik:
                logging.info(f"{sym}: {label} is empty.")
                continue
            logging.info(f"{sym}: checking {label} = {chosen_cik}")
            facts_data = get_companyfacts(chosen_cik, CACHE_DIR, UA, use_cache=(not args.no_cache), sleep_sec=args.sleep)
            if not facts_data or "facts" not in facts_data:
                logging.info(f"{sym}: {label} returned no CompanyFacts.")
                continue
            report = debug_revenue_candidates(facts_data["facts"], fy=FY)
            print("\n--- Revenue candidates for", sym, f"({label}={chosen_cik}) ---")
            for c in report["candidates"]:
                print(c)
            print("\nBest pick:", report["best"])
        sys.exit(0)
    ...
    for idx, row in tqdm(df_in.iterrows(), total=total_rows, desc="Companies"):
        ...
        # fetch CompanyFacts with CIK validation
        facts_data = get_companyfacts(cik10, CACHE_DIR, UA, use_cache=(not args.no_cache), sleep_sec=args.sleep)
        # If no facts and --validate-cik, try the SEC ticker map CIK for the symbol
        if (not facts_data or "facts" not in facts_data) and args.validate_cik:
            mapped_cik = ticker_map.get((row.get("Symbol_norm") or "").upper())
            if mapped_cik and mapped_cik != cik10:
                logging.warning(f"{symbol}: Provided CIK {cik10} had no facts; retrying with mapped CIK {mapped_cik}.")
                facts_data = get_companyfacts(mapped_cik, CACHE_DIR, UA, use_cache=(not args.no_cache), sleep_sec=args.sleep)
                if facts_data and "facts" in facts_data:
                    cik10 = mapped_cik  # use the mapped CIK going forward
        if not facts_data or "facts" not in facts_data:
        ...
        # Currency only exclude if we know it's not USD
        ...
        # Add notes for equity alternatives
        if eq:
    ...
    # Use user-provided filenames inside the chosen output directory
    metrics_csv = os.path.join(args.output_dir, args.output_metrics)
    excl_csv = os.path.join(args.output_dir, args.output_excluded)
    xlsx_path = os.path.join(args.output_dir, args.output_xlsx)
